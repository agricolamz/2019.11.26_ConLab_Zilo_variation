...
## Zilo questionary (`r length(unique(zilo_experiment$speaker_id))` speakers): class attribution
...
* *odoruk’a* 'butterfly' (@butterfly_sg, @butterfly_pl) and *ʁats'a* 'grasshopper' (@grasshopper_sg, @grasshopper_pl) belong to `inan 3` class in Rikvani [@suleymanov57]
...
>- We noticed that *ila-ima* 'parents' can agree with `inan 2` (@parents).
>- We observed different dative morphemes with masculine experiencers -*j*/-*ɬu* (@dative_father, @dative_cow)
>- We observed variability in the ability of the cardinal numeral marker to agree with noun class (@cardinal), though the experimental data showed no variation
>- We noticed variability in the formation of plural forms, but the stimuli selected for this topic (@plural_heel, @plural_soninlow) unfortunately did not vary at all.
>- There are two past copulas *b-ik'o* and *b-iʁi*, the distribution of which is unclear
