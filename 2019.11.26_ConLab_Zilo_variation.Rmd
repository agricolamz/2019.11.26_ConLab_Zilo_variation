...
date: |
    | 26 November 2019, Happy Birthday Ferdinand de Saussure!
    |
    |
...
>- In variationism ([@labov63] on Martha's Vineyard /ai/ ~ /au/, [@trudgill74] on Norwich speech, [@wolfram69] on Detroit Afro-American speech) researchers get mad about social stratification, mostly urban.

But what about other comunities?
...
## `r length(unique(zilo_experiment$speaker_id))` Zilo speakers were asked to translate:

(@butterfly_sg) 'big butterfly'
(@butterfly_pl) 'big butterflys'
(@grasshopper_sg) 'big grasshopper'
(@grasshopper_pl) 'big grasshoppers'
(@parents) 'parents left'
(@dative_father) 'pour your father some whater'
(@dative_cow) 'pour your cow some whater'
(@cardinal) 'third girl'
(@verb1) 'On the wedding the bride was beautiful'
(@plural_heel) 'heels'
(@plural_soninlow) 'sons-in-law'
(@verb2) 'they are eating'
(@verb3) 'when he come, we will eat'
(@lexic_1) 'rainbow'
(@lexic_2) 'north'
(@lexic_3) 'thousand'

## Zilo questionary (`r length(unique(zilo_experiment$speaker_id))` speakers): class atribution

```{r}
read_csv("data/classes_in_andi.csv")
```

* *odoruk’a* 'butterfly' (@butterfly_sg, @butterfly_pl) and *ʁats'a* 'grasshopper' (@grasshopper_sg, @grasshopper_pl) belong to `-an3` class in Rikwani [@suleymanov57]
* *odoruk’a* 'butterfly' (@butterfly_sg) is one of the most variable words from [@moroz19]

## Zilo questionary (`r length(unique(zilo_experiment$speaker_id))` speakers): other questions

>- We noticed that *ila-ima* 'parents' can agree with `-an 2` (@parents).
>- We observed different dative morpheme with masculine experiencer -*j*/-*ɬu* (@dative_father, @dative_cow)
>- It have been claimed that there is variability in cardinal numeral marker (@cardinal) that unfortunately did not appeared in the data...
>- We noticed a variability in plural forms formation, but stimuli selected for this topic (@plural_heel, @plural_soninlow) unfortunately did not vary at all...
>- Samira, say something about (@verb1, @verb3), please
>- An optional plural suffix -*l* can be added to progressive verb forms in -*rado*/-*mado* (@verb2)
>- Examples 'rainbow', 'north', 'thousand' (@lexic_1, @lexic_2, and @lexic_3) were selected since there were known problems in borrowings (@lexic_1, @lexic_2) and borrowing adaptation (@lexic_3): *ozruɡu*, *oziruɡu*, *ozuruɡu*.
...
* in [@moroz19] we asked only singular form
...
zilo_experiment %>% 
  ...
  rename(sg = `1`,
         pl = `2`) %>% 
  mutate(class = c("an", "⸄o⸅", "-an 2"))
```


# \color{colorwhite} Nakh-Daghestanian Fieldwork Survey

## `r nrow(nd_fes)` ND researches were asked to answer about:

* level of education
* linguistic interest
* study linguistics at university
* fieldwork participation when student
* year of finishing study
* place of study and work
* number of people in fieldtrips participated
* number of people in fieldtrips preder to participate
* fildwork goal and the main goal
* elicitation and corpora use
* **number of speakers that researcher should consult with**
* **number of speakers that researcher usually consult with**
* how researcher need to cope with interspeaker variability
* how researcher need to cope with intraspeaker variability
...
experiment_sampling %>% 
  ...
  labs(x = 'number of classes gathered by "researchers"',
       y = 'percantage of "researchers"')+
  ylim(0, 125)
```
...
- **"Average researcher"** might overlook the variation
- Observed variation should be further explored using collected sociolinguistics parameters
- **"Average researcher"** should be eloborated using parameters collected through the Nakh-Daghestanian Fieldwork Survey
- Observed variation is steel just a banch of isolated lexical, phonological and morphological facts... \pause
    - Is it possible to work on some syntax? 
    - Is it possible to find some interaction between different variation variables? \pause
- What does all those results add to the linguistic theory?
